...
def get_usr_old_keys( keyAge ):
    ...
    # Send the Email Notification through SNS only if User Access key exceeding Key_Age        
    if len(usrsWithOldKeys['Users']) > 0:
        snsClient.get_topic_attributes(TopicArn=globalVars['TopicArn'])
        msg = (f"Hi Team\n\nBelow is the list of Secret Key Age of Users which have exceeded {globalVars['key_age']} days:\n\n")
        for user in usrsWithOldKeys['Users']:
            msg += (f"UserName : {user['UserName']}\nKeyAgeInDays:{user['KeyAgeInDays']}\n\n")
        msg += (f"Please rotate the access key before violation of Toyota Cybersecurity Policy baseline requirements.\n\nBest Regards\nPrivate Cloud Team" )
        snsClient.publish(
